...
if [[ "$START_FROM" == "emotion" ]]; then
    ...
    # 진행 중이던 checkpoint 가 있고 metrics.json 이 없으면 resume
    EMOTION_RESUME_ARGS=()
    EMOTION_LATEST_V=$(ls -td "$PROJ/models/emotion/v"* 2>/dev/null | head -1)
    if [ -n "$EMOTION_LATEST_V" ] \
        && [ -s "$EMOTION_LATEST_V/checkpoint_latest/ckpt.pt" ] \
        && [ ! -s "$EMOTION_LATEST_V/metrics.json" ]; then
        EMOTION_RESUME_ARGS=(--resume "$EMOTION_LATEST_V/checkpoint_latest")
        log "🔁 emotion: 진행 중이던 체크포인트 감지 → resume ($EMOTION_LATEST_V/checkpoint_latest)"
    fi
    RUN_PYTHON scripts/train_emotion_model.py \
        --max-epochs 5 --batch-size 32 --save-steps 10000 "${EMOTION_RESUME_ARGS[@]}" \
