...
# 화자분리 동적 스위칭 임계(초). ≤임계 → NeMo 전체재분리(도입부 정확), 초과 → anchor(OOM 방어).
# 실측(2026-06-05): ≤270s VRAM 동거 안전·도입부 분리는 ~210s까지 / 37분 NeMo-full=OOM. 200s 채택.
# 게이트: VOICE_NEMO_FULL_DIAR_ENABLED(nemo_full), VOICE_ANCHOR_DIAR_ENABLED(anchor) — 둘 다 모듈서 os.environ 직독.
VOICE_DIAR_THRESHOLD_SEC = float(os.environ.get("VOICE_DIAR_THRESHOLD_SEC", "200"))
